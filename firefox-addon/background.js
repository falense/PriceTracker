/**
 * PriceTracker Firefox Addon - Background Script
 *
 * Handles:
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === 'getSettings') {
    browser.storage.sync.get({ defaultPriority: 2 }).then(sendResponse);
    return true; // Async response
  }
});

console.log('[PriceTracker] Background script loaded');
