...
def api_retest_pattern_visualization(request):
    ...
    if not domain:
        return HttpResponse('<div class="text-red-600 dark:text-red-400">Domain is required.</div>')
    ...
    try:
        ...
        return HttpResponse('<div class="text-red-600 dark:text-red-400">Pattern not found.</div>')
    except Exception as e:
        logger.exception(f"Error re-testing pattern: {e}")
        return HttpResponse(f'<div class="text-red-600 dark:text-red-400">Error: {str(e)}</div>')
